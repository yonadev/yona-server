...
class RemoveUserTest extends AbstractAppServiceIntegrationTest
{
	...
	def 'Remove Richard and verify that Bob receives a remove buddy message'()
	{
		...
		def buddyDisconnectMessages = getMessagesResponse.responseData._embedded.messages.findAll{ it."@type" == "BuddyDisconnectMessage"}
		...
		getMessagesResponse.responseData._embedded.messages.findAll{ it."@type" == "BuddyConnectRequestMessage"}.size() == 0
	...
	def 'Remove Richard and verify that buddy data is removed after Bob processed the remove buddy message'()
	{
		...
		def disconnectMessage = getResponse.responseData._embedded.messages.findAll{ it."@type" == "BuddyDisconnectMessage"}[0]
	...
	def 'Conflicts for Bob are still processed after the unsubscribe of Richard'()
	{
		...
		appService.deleteUser(richard, message)
		def getResponse = appService.getMessages(bob)
		def processURL = (getResponse.status == 200) ? getResponse.responseData._embedded.messages.findAll{ it."@type" == "BuddyDisconnectMessage"}[0]._links.process.href : null
