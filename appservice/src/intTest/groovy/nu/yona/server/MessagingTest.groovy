...
class MessagingTest extends AbstractAppServiceIntegrationTest
{
	def 'Richard pages through his messages'()
	{
		...
		when:
		def allMessagesResponse = appService.getAnonymousMessages(richard)
		def firstPageMessagesResponse = appService.getAnonymousMessages(richard, [
			"page": 0,
			"size": 2,
			"sort": "creationTime"])
		def secondPageMessagesResponse = appService.getAnonymousMessages(richard, [
			"page": 1,
			"size": 2,
			"sort": "creationTime"])
		...
		firstPageMessagesResponse.responseData._links.self.href == richard.url + appService.ANONYMOUS_MESSAGES_PATH_FRAGMENT + "?page=0&size=2&sort=creationTime"
		...
		secondPageMessagesResponse.responseData._links.self.href == richard.url + appService.ANONYMOUS_MESSAGES_PATH_FRAGMENT + "?page=1&size=2&sort=creationTime"
	...
	def 'Bob tries to delete Richard\'s buddy request before it is processed'()
	{
		...
		!appService.getAnonymousMessages(bob).responseData._embedded.buddyConnectRequestMessages[0]._links.edit
	}
	...
	def 'Bob deletes Richard\'s buddy request after it is processed'()
	{
		...
		def messageDeleteURL = appService.getAnonymousMessages(bob).responseData._embedded.buddyConnectRequestMessages[0]._links.edit.href
	...
	def 'Richard tries to delete Bob\'s buddy acceptance before it is processed'()
	{
		...
		def acceptURL = appService.fetchBuddyConnectRequestMessage(bob).acceptURL
		appService.postMessageActionWithPassword(acceptURL, ["message" : "Yes, great idea!"], bob.password)
		def messageURL = appService.getAnonymousMessages(richard).responseData._embedded.buddyConnectResponseMessages[0]._links.self.href
		...
		response.status == 400
		response.responseData?.code == "error.cannot.delete.unprocessed.message"
		appService.getAnonymousMessages(richard).responseData._embedded.buddyConnectResponseMessages.size() == 1
		!appService.getAnonymousMessages(richard).responseData._embedded.buddyConnectResponseMessages[0]._links.edit
	}
	...
	def 'Richard deletes Bob\'s buddy acceptance after it is processed'()
	{
		...
		def messageDeleteURL = appService.getAnonymousMessages(richard).responseData._embedded.buddyConnectResponseMessages[0]._links.edit.href
	...
	def 'Richard deletes a goal conflict message. After that, Bob still has it'()
	{
		...
		def bob = richardAndBob.bob
		analysisService.postToAnalysisEngine(richard, ["news/media"], "http://www.refdag.nl")
		def messageDeleteURL = appService.getAnonymousMessages(richard).responseData._embedded.goalConflictMessages[0]._links.edit.href
		...
		then:
		response.status == 200
		appService.getAnonymousMessages(richard).responseData._embedded?.goalConflictMessages == null

		appService.getAnonymousMessages(bob).responseData._embedded.goalConflictMessages.size() == 1
	}
	...
	def 'Bob deletes a goal conflict message. After that, Richard still has it'()
	{
		...
		def bob = richardAndBob.bob
		analysisService.postToAnalysisEngine(richard, ["news/media"], "http://www.refdag.nl")
		def messageDeleteURL = appService.getAnonymousMessages(bob).responseData._embedded.goalConflictMessages[0]._links.edit.href
		...
		then:
		response.status == 200
		appService.getAnonymousMessages(richard).responseData._embedded.goalConflictMessages.size() == 1

		appService.getAnonymousMessages(bob).responseData._embedded?.goalConflictMessages == null
	}
}
