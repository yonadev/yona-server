...
public class UserAnonymized extends EntityWithID
{
	...
	public Set<MessageDestination> getAllRelatedDestinations()
	{
		...
		relatedDestinations.addAll(buddiesAnonymized.stream().filter(ba -> ba.getSendingStatus() == Status.ACCEPTED)
				.map(ba -> ba.getUserAnonymized()).map(ua -> ua.getAnonymousDestination()).collect(Collectors.toSet()));
	...
	public BuddyAnonymized getBuddyAnonymizedFromUser(UUID fromUserVPNLoginID)
	{
