...
{
	@Cacheable(value = "goalConflictMessages", key = "{#userLoginID,#goalID,#destination.ID}")
	public GoalConflictMessage fetchLatestGoalConflictMessageForUser(UUID userLoginID, UUID goalID,
			MessageDestination destination, Date minEndTime)
	{
		List<GoalConflictMessage> results = GoalConflictMessage.getGoalConflictMessageRepository()
				.findLatestGoalConflictMessageFromDestination(userLoginID, goalID, destination.getID(), minEndTime,
