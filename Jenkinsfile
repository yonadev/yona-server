pipeline {
	...
	stages {
		...
		stage('Setup test server') {
			...
			steps {
				sh script: 'helm delete yona; kubectl delete -n yona configmaps --all; kubectl delete -n yona job --all; kubectl delete -n yona secrets --all; kubectl delete pvc -n yona --all; true', returnStatus: 0
