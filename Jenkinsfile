	...
	stages {
		...
		stage('Setup test server') {
			...
			steps {
				...
				sh 'while ! $(curl -s -q -f -o /dev/null https://yonadev.github.io/helm-charts/yona-1.2.$BUILD_NUMBER_TO_DEPLOY.tgz) ;do echo Waiting for Helm chart to become available; sleep 5; done'
				sh 'helm delete yona; kubectl delete -n yona configmaps --all; kubectl delete -n yona job --all; kubectl delete -n yona secrets --all; kubectl delete pvc -n yona --all', returnStatus: true
				sh 'helm repo update'
